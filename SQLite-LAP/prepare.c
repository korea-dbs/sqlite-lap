...
//load bitmap_table to DRAM method
/*
void loadBitmapTable(sqlite3 *db){
	const char *sql = "SELECT fippgno,childpg FROM bitmap_table;";
	sqlite3_stmt *stmt = NULL;
	int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
	if(rc != SQLITE_OK){
		fprintf(stderr, "Failed to prepare SELECT from bitmap_table: %s\n",sqlite3_errmsg(db));
		return ;
	}
	while(sqlite3_step(stmt) == SQLITE_ROW){
		int fippgno = sqlite3_column_int(stmt, 0);
		int childpg = sqlite3_column_int(stmt, 1);
		if(bitmap_map[fippgno] == NULL){
			bitmap_map[fippgno] = (FIPPages*)malloc(sizeof(FIPPages));
			bitmap_map[fippgno]->count = 0;
			fprintf(stderr,"%d structure malloc complete\n",fippgno);
		}
		FIPPages *entry = bitmap_map[fippgno];
		if(entry->count < MAX_PREFETCH){
			entry->childpgno[entry->count++] = childpg;
		}else{
			fprintf(stderr,"Warning: Extra entries ignored.\n");
		}
...
static int sqlite3LockAndPrepare(
  ...
  int rc;
  int cnt = 0;
  //toggle of executing method
  static int g_bitmap_loaded = 0;
  static int g_bitmap_loading = 0;
  //end
  ...
  *ppStmt = 0;
  ...
  //execute load bitmap to mem
  /*
  if( rc==SQLITE_OK && !g_bitmap_loaded && !g_bitmap_loading ){
  	g_bitmap_loading = 1;
	loadBitmapTable(db);
	fprintf(stderr,"load bitmaptable complete");
	g_bitmap_loaded = 1;
	g_bitmap_loading = 0;
  }*/
  //end

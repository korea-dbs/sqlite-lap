...
// thread pool init when starting server
...
void thread_pool_init() {
    ...
    for (int i = 0; i < THREAD_POOL_SIZE; i++) {
        pthread_create(&pool->threads[i], NULL, prefetch_worker, (void*)pool);
    }
}
//end
...
#endif
//init iouring when server start
  ...
  if (!ring_initialized) {
  	if(io_uring_queue_init_params(512,&ring,&params)<0){
		...
		ring_initialized = 1;
		fprintf(stderr, "ring init IAM-nomem\n");
		thread_pool_init();
		fprintf(stderr, "THD-Pool init\n");
	}
...
//exit iouring when server shutdown

	if (ring_initialized) {
      		io_uring_queue_exit(&ring);
		fprintf(stderr, "ring exit\n");
      		ring_initialized = 0;
  	}
...
int sqlite3_close(sqlite3 *db){
	// confirm hit ratio when shutdown sqlite engine
	fprintf(stderr,"close func hit : %llu ,miss : %llu ",hit, miss);
	//end

...
static int getPageNormal(
  ...
  if( read_ahead == 0 && rc == SQLITE_OK /*&& pPg != 0 && pBase == 0*/ ){
    
    Pgno nextPgno;
    for(int i = 1; i <= 1; i++){
      nextPgno = pgno + i;
      fprintf(stderr,"readahead : %d - %d \n ",nextPgno,nextPgno+4);
      
      if( nextPgno <= pPager->dbSize ){
        DbPage *pRaPage = 0;
        
        read_ahead = 1;
        getPageNormal(pPager, nextPgno, &pRaPage, flags);
        read_ahead = 0;
        if( pRaPage ){
          sqlite3PagerUnref(pRaPage); 
        }
      }
    }
  }

...
int pPgindex = 1;
//end
//finconstruct: guards against re-entering moveToChild while the
//INSERT INTO bitmap_table below is itself walking the b-tree.
static int sqltoggle = 0;
/*
  ...
  if( rc==SQLITE_OK && amt>0 ){
    ...
    */
    ...
    while( nextPage ){
      ...
      //
      //pgcnt++;
      //fprintf(stderr,"ovflpageno :%d, page cnt : %d\n", pCur->aOverflow[iIdx],pgcnt);
      //end
...
static int moveToChild(BtCursor *pCur, u32 newPgno){
  ...
  //finconstruct: pParent's child just turned out to be a genuine leaf
  //(0x0d). Promote pParent to a FIN (Final Interior Node, 0x07) and
  //record the (fippgno, childpg) edge in bitmap_table.
  if( pCur->pPage->aData[0]==0x0d && pParent->pgno!=1 && sqltoggle==0 ){
    ...
    if( sqlite3_exec(db, zSql, 0, 0, &zErr)!=SQLITE_OK ){
      fprintf(stderr, "finconstruct: bitmap_table insert failed: %s\n", zErr?zErr:"(unknown)");
      sqlite3_free(zErr);
    }
    sqltoggle = 0;
  }

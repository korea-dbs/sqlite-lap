...
static int seekAndRead(unixFile *id, sqlite3_int64 offset, void *pBuf, int cnt){
 ...
 // fprintf(stderr,"return : %d offset : %lld cnt : %d pBUf : %p\n",got+prior, offset, cnt, (void*)pBuf);
  return got+prior;
}
...
static int unixRead(
  ...
  int got;
  assert( id );
  assert( offset>=0 );//If this is a database file (not a journal, super-journal or temp
  assert( amt>0 );//** file), the bytes in the locking range should never be read or written.
 ...
 // time_taken = (end.tv_sec - start.tv_sec)+ (end.tv_nsec - start.tv_nsec)/1000000000;
 // printf("time : %.9f capacity : %d\n",time_taken,got);
